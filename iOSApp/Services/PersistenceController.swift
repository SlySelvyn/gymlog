...
final class PersistenceController {
    ...
    func saveWorkout(_ record: WorkoutRecord) throws {
        ...
        for ex in record.exercises {
            ...
            for s in ex.sets {
                ...
                se.setValue(s.reps.map { Int32($0) }, forKey: "reps")
                se.setValue(s.rpe, forKey: "rpe")
                se.setValue(s.rir.map { Int32($0) }, forKey: "rir")
                se.setValue(s.timestamp, forKey: "timestamp")
                se.setValue(s.restSeconds.map { Int32($0) }, forKey: "restSeconds")
                setSet.add(se)
            }
    ...
    func saveRoutine(_ record: RoutineRecord) throws {
        ...
        for item in record.items {
            ...
            i.setValue(item.targetSets.map { Int32($0) }, forKey: "targetSets")
            i.setValue(item.targetReps.map { Int32($0) }, forKey: "targetReps")
